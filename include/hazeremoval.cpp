...
bool CHazeRemoval::InitProc(int width, int height, int nChannels) {
	...
	InitPrams(7, 0.95, 0.1, 60, 0.001);

	if (width > 0 && height > 0 && nChannels == 3) ret = true;
	return ret;
}

void CHazeRemoval::InitPrams(int radius, double omega, double t0, int r, double eps) {
	m_radius	= radius;
	m_omega		= omega;
	m_t0		= t0;
	m_r			= r;
...
bool CHazeRemoval::Process(const unsigned char* indata, unsigned char* outdata, int width, int height, int nChannels) {
	...
	get_dark_channel(p_src, tmp_vec, rows, cols, channels, m_radius);
	get_air_light(p_src, tmp_vec, p_Alight, rows, cols, channels);
	get_transmission(p_src, p_tran, p_Alight, rows, cols, channels, m_radius, m_omega);
	guided_filter(p_src, p_tran, p_gtran, m_r, m_eps);
	recover(p_src, p_gtran, p_dst, p_Alight, rows, cols, channels, m_t0);
